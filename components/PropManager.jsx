...
// ─── SUPABASE CONFIG ─────────────────────────────────────────────────────────
// 👇 PASTE YOUR VALUES HERE after completing Supabase setup
...
const sb = {
  from: (table) => ({
    ...
    delete: async () => {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/${table}?id=neq.null`, {
        method: "DELETE",
        headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` }
      });
...
const DB_READY = SUPABASE_URL !== "https://YOUR_PROJECT_ID.supabase.co";
...
const StatCard = ({ label, value, icon, color, sub }) => {
  const colors = { indigo: "bg-indigo-50 text-indigo-600", emerald: "bg-emerald-50 text-emerald-600", amber: "bg-amber-50 text-amber-600", rose: "bg-rose-50 text-rose-600", blue: "bg-blue-50 text-blue-600" };
  return (
...
const Select = ({ children, ...props }) => <select {...props} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-slate-50">{children}</select>;
...
export default function App() {
  ...
  const loadAll = async () => {
    ...
    if (!pr.error) setProperties(pr.data.map(mapProperty));
    if (!tr.error) setTenants(tr.data.map(mapTenant));
    if (!cr.error) setCharges(cr.data.map(mapCharge));
    if (!pmr.error) setPayments(pmr.data.map(mapPayment));
    setLoading(false);
  };
  ...
  const savePayment = async () => {
    ...
    setSyncing(true);
    if (DB_READY) await sb.from("payments").insert([toDBPayment(p)]);
  ...
  const saveTenant = async () => {
    ...
    setSyncing(true);
    if (tForm.id) {
      if (DB_READY) await sb.from("tenants").update(toDBTenant(tForm), "id", tForm.id);
      ...
      if (DB_READY) {
        await sb.from("tenants").insert([toDBTenant(t)]);
        await sb.from("properties").update({ status: "Occupied" }, "id", t.propertyId);
      }
  ...
  const saveProperty = async () => {
    ...
    setSyncing(true);
    if (pForm.id) {
      if (DB_READY) await sb.from("properties").update(toDBProperty(pForm), "id", pForm.id);
      setProperties(prev => prev.map(p => p.id === pForm.id ? pForm : p));
    } else {
      const prop = { id: uid("P"), status: "Vacant", ...pForm };
      if (DB_READY) await sb.from("properties").insert([toDBProperty(prop)]);
  ...
  const saveCharge = async () => {
    ...
    setSyncing(true);
    if (DB_READY) await sb.from("charges").insert([toDBCharge(c)]);
  ...
  const generateMonthlyBilling = async (period) => {
    ...
    setSyncing(true);
    if (DB_READY) await sb.from("charges").insert(newC.map(toDBCharge));
  ...
  const handleReset = async () => {
    ...
    if (DB_READY) {
      ...
      await sb.from("charges").delete();
      await sb.from("tenants").delete();
      await sb.from("properties").delete();
    }
    setProperties([]); setTenants([]); setCharges([]); setPayments([]);
    setResetPwd(""); setResetErr(false);
    setSyncing(false);
    closeModal();
  };
  ...
  const printSOA = (tenant) => {
    ...
    printHTML(`
      <div class="header"><div><div class="logo">🏢 PropManager</div><div class="owner">${OWNER_NAME}</div><p style="color:#64748b;font-size:12px;margin-top:4px">Statement of Account</p></div><div style="text-align:right;font-size:11px;color:#64748b">Generated: ${fmtDate(today())}</div></div>
      <table style="margin-bottom:16px;border:none"><tr><td style="border:none;padding:2px 0"><b>Tenant:</b> ${tenant.name}</td><td style="border:none;padding:2px 0"><b>Unit:</b> ${prop?.name || "—"}</td></tr><tr><td style="border:none;padding:2px 0"><b>Email:</b> ${tenant.email}</td><td style="border:none;padding:2px 0"><b>Lease End:</b> ${fmtDate(tenant.leaseEnd)}</td></tr></table>
      <table><thead><tr><th>Date</th><th>Description</th><th class="right">Debit</th><th class="right">Credit</th><th class="right">Balance</th></tr></thead><tbody>${rows}</tbody><tr class="total-row"><td colspan="3"></td><td class="right">Outstanding:</td><td class="right">${fmtCurrency(balance)}</td></tr></table>`,
    `SOA — ${tenant.name}`);
  };
  const printReceipt = (payment) => {
    ...
    printHTML(`
      <div class="header"><div><div class="logo">🏢 PropManager</div><div class="owner">${OWNER_NAME}</div><p style="color:#64748b;font-size:12px;margin-top:4px">Official Receipt</p></div><div style="text-align:right"><b>OR#:</b> ${payment.id}<br><span style="color:#64748b;font-size:11px">${fmtDate(payment.date)}</span></div></div>
      <table style="border:none"><tr><td style="border:none;padding:4px 0;width:50%"><b>Received from:</b><br>${tenant?.name || "—"}</td><td style="border:none;padding:4px 0"><b>Unit:</b><br>${prop?.name || "—"}</td></tr><tr><td style="border:none;padding:4px 0"><b>Amount Paid:</b><br><span style="font-size:22px;font-weight:800;color:#4f46e5">${fmtCurrency(payment.amount)}</span></td><td style="border:none;padding:4px 0"><b>Method:</b><br>${payment.method}</td></tr><tr><td style="border:none;padding:4px 0"><b>Reference:</b><br>${payment.reference || "—"}</td><td style="border:none;padding:4px 0"><b>Remaining Balance:</b><br><span class="bold ${balance > 0 ? "red" : "green"}">${fmtCurrency(balance)}</span></td></tr></table>
      ${payment.notes ? `<p style="margin-top:16px;padding:12px;background:#f8fafc;border-radius:8px;font-size:12px"><b>Notes:</b> ${payment.notes}</p>` : ""}
  ...
  const printIncomeReport = () => {
    ...
    printHTML(`
      <div class="header"><div><div class="logo">🏢 PropManager</div><div class="owner">${OWNER_NAME}</div><p style="color:#64748b;font-size:12px;margin-top:4px">Income Report</p></div><div style="font-size:11px;color:#64748b">As of ${fmtDate(today())}</div></div>
      <table><thead><tr><th>Period</th><th class="right">Collected</th></tr></thead><tbody>${rows}</tbody><tr class="total-row"><td>Total</td><td class="right">${fmtCurrency(total)}</td></tr></table>`,
    "Income Report");
  };
  ...
  const NotConfigured = () => (
    <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-4 flex gap-3">
      <Icon name="alert" size={20} className="text-amber-500 flex-shrink-0 mt-0.5" />
      <div>
        <p className="text-sm font-bold text-amber-800">Supabase not connected yet</p>
        <p className="text-xs text-amber-700 mt-0.5">Data is stored in memory only. Open the .jsx file and paste your Supabase URL and anon key at the top to enable persistent storage.</p>
      </div>
    </div>
  );
  ...
  const SyncBar = () => syncing ? (
    <div className="fixed top-14 left-0 right-0 z-50 bg-indigo-600 text-white text-xs font-semibold text-center py-1.5 flex items-center justify-center gap-2">
      <Icon name="cloud" size={13} /> Saving to Supabase…
    </div>
  ) : null;
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-4">
      ...
      <div className="text-center">
        <p className="font-bold text-slate-800">PropManager</p>
        <p className="text-xs text-indigo-400 font-semibold">{OWNER_NAME}</p>
  ...
  const DashboardView = () => {
    ...
    const expiringLeases = tenants.filter(t => { const d = (new Date(t.leaseEnd) - new Date()) / 86400000; return d > 0 && d < 90; }).sort((a, b) => (a.leaseEnd || "").localeCompare(b.leaseEnd || ""));
    return (
      <div className="space-y-4">
        {!DB_READY && <NotConfigured />}
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-500 rounded-2xl px-5 py-4 shadow-md">
          ...
          <p className="text-indigo-200 text-sm">Portfolio Overview</p>
        </div>
        ...
        <Card className="p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setModal({ type: "payment" })} className="flex items-center gap-2 bg-indigo-600 text-white rounded-xl px-4 py-3 text-sm font-semibold hover:bg-indigo-700 transition-colors"><Icon name="wallet" size={16} /> Record Payment</button>
            <button onClick={() => setModal({ type: "charge" })}  className="flex items-center gap-2 bg-slate-800 text-white rounded-xl px-4 py-3 text-sm font-semibold hover:bg-slate-900 transition-colors"><Icon name="plus" size={16} /> Add Charge</button>
            <button onClick={() => { const p = prompt("Billing period (YYYY-MM):"); if (p) generateMonthlyBilling(p); }} className="flex items-center gap-2 border-2 border-indigo-200 text-indigo-700 rounded-xl px-4 py-3 text-sm font-semibold hover:bg-indigo-50 transition-colors"><Icon name="calendar" size={16} /> Generate Billing</button>
            <button onClick={printIncomeReport} className="flex items-center gap-2 border-2 border-slate-200 text-slate-700 rounded-xl px-4 py-3 text-sm font-semibold hover:bg-slate-50 transition-colors"><Icon name="download" size={16} /> Income Report</button>
            <button onClick={loadAll} className="flex items-center justify-center gap-2 border-2 border-emerald-200 text-emerald-700 rounded-xl px-4 py-2.5 text-sm font-semibold hover:bg-emerald-50 transition-colors"><Icon name="refresh" size={15} /> Refresh Data</button>
            <button onClick={() => { setResetPwd(""); setResetErr(false); setModal({ type: "reset" }); }} className="flex items-center justify-center gap-2 border-2 border-rose-200 text-rose-400 rounded-xl px-4 py-2.5 text-sm font-semibold hover:bg-rose-50 transition-colors"><Icon name="trash" size={15} /> Reset All Data</button>
          </div>
        </Card>
        <Card>
          ...
          {recentPay.map(p => {
            ...
            return (
              <div key={p.id} className="flex items-center justify-between p-3 border-b border-slate-50 last:border-0">
                <div className="min-w-0"><p className="text-sm font-semibold text-slate-800 truncate">{t?.name || "—"}</p><p className="text-xs text-slate-400">{fmtDate(p.date)} · {p.method}</p></div>
                <div className="text-right ml-2 flex-shrink-0"><p className="text-sm font-bold text-emerald-600">{fmtCurrency(p.amount)}</p><button onClick={() => printReceipt(p)} className="text-xs text-indigo-500 hover:underline">Receipt</button></div>
              </div>
            );
          })}
        </Card>
  ...
  const TenantsView = () => {
    ...
    if (viewTenant) {
      ...
      const ledger = getLedger(t.id, charges, payments);
      return (
        <div className="space-y-4">
          <OwnerWatermark />
          <button onClick={() => setViewTenant(null)} className="flex items-center gap-2 text-sm text-indigo-600 font-semibold">← Back to Tenants</button>
          <Card className="p-4">
            <div className="flex items-start justify-between mb-3">
              <div><h2 className="text-lg font-bold text-slate-800">{t.name}</h2><p className="text-sm text-slate-500">{prop?.name}</p></div>
              <span className={`text-sm font-bold px-3 py-1 rounded-full ${balance > 0 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>{balance > 0 ? "Owes " : "Credit "}{fmtCurrency(Math.abs(balance))}</span>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div><span className="text-slate-400">Security Deposit</span><br /><span className="font-medium">{fmtCurrency(t.deposit)}</span></div>
            </div>
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={() => { setModal({ type: "payment" }); setPayForm(f => ({ ...f, tenantId: t.id })); }} className="flex-1 flex items-center justify-center gap-2 border-2 border-indigo-200 text-indigo-700 rounded-xl py-2.5 text-sm font-semibold"><Icon name="wallet" size={14} /> Record Payment</button>
            </div>
          </Card>
          <Card>
            <div className="p-4 border-b border-slate-100"><p className="font-semibold text-slate-800 text-sm">Tenant Ledger</p></div>
                ...
                <tbody>
                  ...
                  {ledger.map((e, i) => (
                    <tr key={i} className="border-t border-slate-50">
                      <td className="p-3 text-slate-600">{fmtDate(e.date)}</td>
                      <td className="p-3 text-slate-800 font-medium">{e.desc}</td>
                      <td className="p-3 text-right text-rose-600">{e.debit ? fmtCurrency(e.debit) : "—"}</td>
                      <td className="p-3 text-right text-emerald-600">{e.credit ? fmtCurrency(e.credit) : "—"}</td>
                      <td className={`p-3 text-right font-bold ${e.balance > 0 ? "text-rose-600" : "text-emerald-600"}`}>{fmtCurrency(e.balance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold text-slate-800">Tenants</h1><p className="text-sm text-slate-500">{tenants.length} tenants</p></div>
          <button onClick={() => { setTForm({}); setModal({ type: "tenant" }); }} className="flex items-center gap-1.5 bg-indigo-600 text-white rounded-xl px-4 py-2.5 text-sm font-semibold"><Icon name="plus" size={16} /> Add</button>
        </div>
        ...
        <div className="space-y-2">
          {tenants.length === 0 && <p className="text-center text-slate-400 text-sm py-8">No tenants yet. Add one!</p>}
          {filtered.map(t => {
            const prop = properties.find(p => p.id === t.propertyId);
            const balance = getTenantBalance(t.id, charges, payments);
            return (
              <Card key={t.id} className="p-4">
                <div className="flex items-start justify-between">
                  <div className="min-w-0 flex-1"><p className="font-semibold text-slate-800 truncate">{t.name}</p><p className="text-xs text-slate-500 mt-0.5">{prop?.name} · {t.phone}</p><p className="text-xs text-slate-400 mt-0.5">Lease ends {fmtDate(t.leaseEnd)}</p></div>
                  <div className="text-right ml-2 flex-shrink-0"><p className={`text-sm font-bold ${balance > 0 ? "text-rose-600" : "text-emerald-600"}`}>{fmtCurrency(Math.abs(balance))}</p><p className="text-xs text-slate-400">{balance > 0 ? "balance due" : "credit"}</p></div>
                </div>
                <div className="flex gap-2 mt-3">
                  <button onClick={() => setViewTenant(t)} className="flex-1 text-xs text-indigo-600 font-semibold border border-indigo-200 rounded-lg py-1.5 hover:bg-indigo-50">View</button>
                  <button onClick={() => { setTForm({ ...t }); setModal({ type: "tenant" }); }} className="flex-1 text-xs text-slate-600 font-semibold border border-slate-200 rounded-lg py-1.5 hover:bg-slate-50">Edit</button>
                  <button onClick={() => printSOA(t)} className="flex-1 text-xs text-slate-600 font-semibold border border-slate-200 rounded-lg py-1.5 hover:bg-slate-50">SOA</button>
                </div>
              </Card>
            );
          })}
  ...
  const PropertiesView = () => {
    ...
    return (
      <div className="space-y-4">
        <OwnerWatermark />
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold text-slate-800">Properties</h1><p className="text-sm text-slate-500">{properties.length} units</p></div>
          <button onClick={() => { setPForm({}); setModal({ type: "property" }); }} className="flex items-center gap-1.5 bg-indigo-600 text-white rounded-xl px-4 py-2.5 text-sm font-semibold"><Icon name="plus" size={16} /> Add</button>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-1">
          {["All", "Occupied", "Vacant", "Reserved", "Under Maintenance"].map(s => <button key={s} onClick={() => setFilter(s)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${filter === s ? "bg-indigo-600 text-white" : "bg-white border border-slate-200 text-slate-600"}`}>{s}</button>)}
        </div>
        <div className="grid gap-3">
          {properties.length === 0 && <p className="text-center text-slate-400 text-sm py-8">No units yet. Add one!</p>}
          {filtered.map(p => {
  ...
  const PaymentsView = () => {
    ...
    const filtered = sorted.filter(p => { const t = tenants.find(t => t.id === p.tenantId); return (t?.name || "").toLowerCase().includes(search.toLowerCase()) || (p.reference || "").toLowerCase().includes(search.toLowerCase()); });
    return (
      <div className="space-y-4">
        <OwnerWatermark />
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold text-slate-800">Payments</h1><p className="text-sm text-slate-500">{payments.length} transactions</p></div>
          <button onClick={() => setModal({ type: "payment" })} className="flex items-center gap-1.5 bg-indigo-600 text-white rounded-xl px-4 py-2.5 text-sm font-semibold"><Icon name="plus" size={16} /> Record</button>
        </div>
        <div className="relative"><Icon name="search" size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by tenant or reference…" className="w-full border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white" /></div>
        <div className="space-y-2">
          ...
          {filtered.map(p => {
            ...
            return (
              <Card key={p.id} className="p-4">
                <div className="flex items-start justify-between">
                  <div className="min-w-0 flex-1"><p className="font-semibold text-slate-800 truncate">{t?.name || "—"}</p><p className="text-xs text-slate-500">{fmtDate(p.date)} · {p.method}</p>{p.reference && <p className="text-xs text-slate-400">Ref: {p.reference}</p>}{p.notes && <p className="text-xs text-slate-400 italic mt-0.5">{p.notes}</p>}</div>
                  <div className="text-right ml-2 flex-shrink-0"><p className="text-base font-bold text-emerald-600">{fmtCurrency(p.amount)}</p><button onClick={() => printReceipt(p)} className="text-xs text-indigo-500 hover:underline font-medium">Print OR</button></div>
                </div>
              </Card>
            );
          })}
  ...
  const ChargesView = () => {
    ...
    const filtered = sorted.filter(c => { const t = tenants.find(t => t.id === c.tenantId); return (t?.name || "").toLowerCase().includes(search.toLowerCase()) || (c.type || "").toLowerCase().includes(search.toLowerCase()); });
    return (
      <div className="space-y-4">
        <OwnerWatermark />
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold text-slate-800">Charges</h1><p className="text-sm text-slate-500">{charges.length} records</p></div>
          <button onClick={() => setModal({ type: "charge" })} className="flex items-center gap-1.5 bg-indigo-600 text-white rounded-xl px-4 py-2.5 text-sm font-semibold"><Icon name="plus" size={16} /> Add</button>
        </div>
        <div className="relative"><Icon name="search" size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search charges…" className="w-full border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white" /></div>
        <div className="space-y-2">
          ...
          {filtered.map(c => {
            ...
            return (
              <Card key={c.id} className="p-4">
                <div className="flex items-start justify-between">
                  <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="font-semibold text-slate-800 truncate">{t?.name || "—"}</p><span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full flex-shrink-0">{c.type}</span></div><p className="text-xs text-slate-500">{fmtDate(c.date)} · {c.period}</p><p className="text-xs text-slate-400 italic mt-0.5">{c.remarks}</p></div>
                  <p className="text-base font-bold text-rose-600 ml-2 flex-shrink-0">{fmtCurrency(c.amount)}</p>
                </div>
              </Card>
            );
          })}
  ...
  const ReportsView = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        <Card className="p-4">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Export Reports</p>
          <div className="grid grid-cols-1 gap-2">
            <button onClick={printIncomeReport} className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50"><div className="p-2 bg-indigo-50 rounded-lg"><Icon name="trending" size={16} className="text-indigo-600" /></div> Monthly Income Report</button>
            <button onClick={printOutstanding} className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50"><div className="p-2 bg-rose-50 rounded-lg"><Icon name="alert" size={16} className="text-rose-600" /></div> Outstanding Balances Report</button>
            {tenants.map(t => (<button key={t.id} onClick={() => printSOA(t)} className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50"><div className="p-2 bg-emerald-50 rounded-lg"><Icon name="receipt" size={16} className="text-emerald-600" /></div> SOA — {t.name}</button>))}
          </div>
        </Card>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-2xl mx-auto flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            {DB_READY && <div className="w-2 h-2 rounded-full bg-emerald-400" title="Connected to Supabase" />}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-100 shadow-lg">
        <div className="max-w-2xl mx-auto flex">
          {navItems.map(n => (<button key={n.id} onClick={() => setTab(n.id)} className={`flex-1 flex flex-col items-center gap-1 py-2.5 transition-colors ${tab === n.id ? "text-indigo-600" : "text-slate-400 hover:text-slate-600"}`}><Icon name={n.icon} size={tab === n.id ? 22 : 20} /><span className="text-[10px] font-semibold">{n.label}</span></button>))}
        </div>
      </div>
      ...
      {modal?.type === "payment" && (
        <Modal title="Record Payment" onClose={closeModal}>
          <Field label="Tenant *"><Select value={payForm.tenantId} onChange={e => setPayForm(f => ({ ...f, tenantId: e.target.value }))}><option value="">Select tenant…</option>{tenants.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</Select></Field>
          ...
          <Field label="Payment Method"><Select value={payForm.method} onChange={e => setPayForm(f => ({ ...f, method: e.target.value }))}>{["Cash", "Bank Transfer", "GCash", "Maya", "Cheque", "Other"].map(m => <option key={m}>{m}</option>)}</Select></Field>
          <Field label="Reference Number"><Input placeholder="e.g. BT-2025060101" value={payForm.reference} onChange={e => setPayForm(f => ({ ...f, reference: e.target.value }))} /></Field>
          <Field label="Notes"><Textarea placeholder="Optional notes…" value={payForm.notes} onChange={e => setPayForm(f => ({ ...f, notes: e.target.value }))} /></Field>
          <button onClick={savePayment} className="w-full bg-indigo-600 text-white rounded-xl py-3 text-sm font-bold mt-2 hover:bg-indigo-700">Save Payment & Generate Receipt</button>
        </Modal>
      )}

      {/* Tenant Modal */}
      {modal?.type === "tenant" && (
        <Modal title={tForm.id ? "Edit Tenant" : "Add Tenant"} onClose={closeModal}>
          ...
          <Field label="Unit *"><Select value={tForm.propertyId || ""} onChange={e => setTForm(f => ({ ...f, propertyId: e.target.value }))}><option value="">Select unit…</option>{properties.filter(p => p.status !== "Occupied" || p.id === tForm.propertyId).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</Select></Field>
          <div className="grid grid-cols-2 gap-2">
          ...
          <button onClick={saveTenant} className="w-full bg-indigo-600 text-white rounded-xl py-3 text-sm font-bold mt-2 hover:bg-indigo-700">{tForm.id ? "Update Tenant" : "Add Tenant"}</button>
        </Modal>
      )}

      {/* Property Modal */}
      {modal?.type === "property" && (
        <Modal title={pForm.id ? "Edit Unit" : "Add Unit"} onClose={closeModal}>
          ...
          <Field label="Status"><Select value={pForm.status || "Vacant"} onChange={e => setPForm(f => ({ ...f, status: e.target.value }))}>{["Vacant", "Occupied", "Reserved", "Under Maintenance"].map(s => <option key={s}>{s}</option>)}</Select></Field>
          <button onClick={saveProperty} className="w-full bg-indigo-600 text-white rounded-xl py-3 text-sm font-bold mt-2 hover:bg-indigo-700">{pForm.id ? "Update Unit" : "Add Unit"}</button>
        </Modal>
      )}

      {/* Charge Modal */}
      {modal?.type === "charge" && (
        <Modal title="Add Charge" onClose={closeModal}>
          <Field label="Tenant *"><Select value={cForm.tenantId} onChange={e => setCForm(f => ({ ...f, tenantId: e.target.value }))}><option value="">Select tenant…</option>{tenants.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</Select></Field>
          <div className="grid grid-cols-2 gap-2">
            <Field label="Billing Period"><Input type="month" value={cForm.period} onChange={e => setCForm(f => ({ ...f, period: e.target.value }))} /></Field>
            <Field label="Date"><Input type="date" value={cForm.date} onChange={e => setCForm(f => ({ ...f, date: e.target.value }))} /></Field>
          </div>
          <Field label="Charge Type"><Select value={cForm.type} onChange={e => setCForm(f => ({ ...f, type: e.target.value }))}>{["Rent", "Dues", "Parking", "Utility", "Penalty", "Other"].map(t => <option key={t}>{t}</option>)}</Select></Field>
      ...
      {/* Reset Modal */}
      {modal?.type === "reset" && (
        <Modal title="Reset All Data" onClose={closeModal}>
          <div className="flex flex-col items-center text-center gap-3 py-2">
            ...
            <div><p className="font-bold text-slate-800 text-base">This will erase everything.</p><p className="text-sm text-slate-500 mt-1">All properties, tenants, charges, and payments will be permanently deleted from Supabase. Enter the password to confirm.</p></div>
          </div>
          <div className="mt-4">
            <Field label="Password">
              <div className="relative">
                <Icon name="lock" size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input type="password" value={resetPwd} onChange={e => { setResetPwd(e.target.value); setResetErr(false); }} onKeyDown={e => e.key === "Enter" && handleReset()} placeholder="Enter password…" className={`w-full border rounded-xl pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 bg-slate-50 ${resetErr ? "border-rose-400 focus:ring-rose-300" : "border-slate-200 focus:ring-indigo-400"}`} />
              </div>
              {resetErr && <p className="text-xs text-rose-500 mt-1 font-semibold">Incorrect password. Try again.</p>}
            </Field>
            <button onClick={handleReset} className="w-full bg-rose-600 text-white rounded-xl py-3 text-sm font-bold mt-1 hover:bg-rose-700">Delete All Data</button>
